...
- `uv run gofer select PROJ-123 feature/branch` — select a branch for a pending ticket
- `uv run gofer select PROJ-123 --fresh` — start with a fresh branch
- `uv run gofer select PROJ-123 --list` — list available branches for a pending ticket
...
```
src/gofer/
├── main.py          # Entry point, poll loop, CLI (run/approve/reject/select/do subcommands)
...
├── worktree.py      # Git worktree create/remove, list_remote_branches, with subprocess timeouts
├── gate.py          # Two-stage complexity gate (heuristics skip Stage 2 if flagged)
├── approval.py      # File-based approval queue + branch selection (pending_approvals.json)
